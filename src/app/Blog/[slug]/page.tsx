...
const blogs: Blog[] = [
  ...
  },
  {
    ...
    " Berkshire County, Massachusetts, and, in the upper part, the so-called Cockeysville Marble.",
     ...
     " tall, according to U.S. National Geodetic Survey measurements in 2013–2014.[B] It is the tallest monumental column",      
    ],
...
export default function BlogDetail({ params }: BlogDetailProps) {
    if (!params?.slug) {
      notFound(); // Agar slug missing hai, seedha notFound call kar do
    }
  
    const blog = blogs.find(
      (b) => b.slug.toLowerCase() === params.slug.toLowerCase() // Matching slug correctly
    );
  
    if (!blog) {
      notFound(); // Blog na mila to bhi notFound call karo
    }
    ...
    return (
      <div className="min-h-screen bg-gray-900 text-white">
        {/* Back to Blog Button */}
        <div className="p-4">
          <Link href="/Blog" className="text-blue-400 hover:text-blue-600 text-sm flex items-center">
            ← Back to Blog
          </Link>
        </div>
